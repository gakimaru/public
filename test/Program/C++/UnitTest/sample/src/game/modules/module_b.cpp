...
UT_BEGIN(CModuleB, UT_GROUP_USER_B, UnitTest::UT_ATTR_AUTO)//モジュール名とグループIDを指定　※特定のモジュール名やグループIDに絞ったテストを実行可能
{
	CModuleB _CModuleB;
	int out_par = 0;
...
UT_END()
UT_BEGIN(CModuleC, UT_GROUP_USER_B, UnitTest::UT_ATTR_NOAUTO)//モジュール名とグループIDを指定　※特定のモジュール名やグループIDに絞ったテストを実行可能　※非自動実行属性付与
{
	CModuleC _CModuleC;
	float out_par1 = 0.f;
	float out_par2 = 0.f;
