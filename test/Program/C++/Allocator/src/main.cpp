...
public:
	...
	virtual E_ORDERED getDefaultOrdered() const = 0;//デフォルトのスタック順を取得
...
public:
	...
	E_ORDERED getDefaultOrdered() const override { return NORMAL; }//デフォルトのスタック順を取得
...
public:
	...
	E_ORDERED getDefaultOrdered() const override { return m_defaultOrdered; }//デフォルトのスタック順を取得
...
public:
	...
	//コンストラクタ
	CDualStackAllocAdp(CDualStackAllocator& stack, IStackAllocator::E_ORDERED ordered = IStackAllocator::DEFAULT) :
...
public:
	...
	CTempPolyAllocator() = delete;
	//コンストラクタ
	CTempPolyAllocator(CDualStackAllocator& allocator, const IStackAllocator::E_ORDERED ordered = IStackAllocator::DEFAULT) :
...
#include <unordered_map>//C++11のハッシュテーブル用
class CTest8
{
public:
	//型
...
public:
	...
	const DATA* find(const crc32_t name_crc) const
	{
		...
		auto& obj = m_table->find(name_crc);
		if (obj == m_table->cend())
			return nullptr;
		return &obj->second;
	}
...
//※メモリ使用状況を確認するためにマーカーを表示
//配置new
void* operator new(const std::size_t size, CSmartStackAllocator& allocator, const std::size_t align, const CSmartStackAllocator::E_ORDERED ordered = CSmartStackAllocator::DEFAULT) throw()
{
	printf("placement new(size=%d, dual_allocator.marker=%d,%d, align=%d, ordered=%d)\n", size, allocator.getMarkerN(), allocator.getMarkerR(), align, ordered);
	return allocator.alloc(ordered, size, align);
}
void* operator new(const std::size_t size, CSmartStackAllocator& allocator, const CSmartStackAllocator::E_ORDERED ordered = CSmartStackAllocator::DEFAULT) throw()
{
	const std::size_t align = IStackAllocator::DEFAULT_ALIGN;
	printf("placement new(size=%d, dual_allocator.marker=%d,%d, (align=%d), ordered=%d)\n", size, allocator.getMarkerN(), allocator.getMarkerR(), align, ordered);
	return allocator.alloc(ordered, size, align);
...
//配置delete
void operator delete(void* p, CSmartStackAllocator& allocator, const std::size_t align, const CSmartStackAllocator::E_ORDERED) throw()
{
	printf("placement delete(p=0x%p, dual_allocator.marker=%d,%d, align)\n", p, allocator.getMarkerN(), allocator.getMarkerR());
	//allocator.back(ordered, p) は実行しない
}
void operator delete(void* p, CSmartStackAllocator& allocator, const CSmartStackAllocator::E_ORDERED) throw()
{
	printf("placement delete(p=0x%p, dual_allocator.marker=%d,%d)\n", p, allocator.getMarkerN(), allocator.getMarkerR());
	//allocator.back(ordered, p) は実行しない
}
//配列版
void operator delete[](void* p, CSmartStackAllocator& allocator, const std::size_t align, const CSmartStackAllocator::E_ORDERED ordered) throw()
{
	printf("placement delete[](p=0x%p, dual_allocator.marker=%d,%d, align)\n", p, allocator.getMarkerN(), allocator.getMarkerR());
	//allocator.back(ordered, p) は実行しない
}
void operator delete[](void* p, CSmartStackAllocator& allocator, const CSmartStackAllocator::E_ORDERED ordered) throw()
{
	const std::size_t align = IStackAllocator::DEFAULT_ALIGN;
	printf("placement delete[](p=0x%p, dual_allocator.marker=%d,%d)\n", p, allocator.getMarkerN(), allocator.getMarkerR());
	//allocator.back(ordered, p) は実行しない
...
class CSmartStackAllocAdp : public IAllocator
{
public:
	//アクセッサ
	...
	std::size_t getUsed() const override{ return m_allocator.getUsed(); }//使用中メモリ量取得
	std::size_t getRemain() const override { return m_allocator.getRemain(); }//残りメモリ量取得
	CSmartStackAllocator& getAllocator() const { return m_allocator; }//アロケータ取得
	CSmartStackAllocator::E_ORDERED getOrdered() const { m_ordered; }//スタック順取得
	void setOrdered(const CSmartStackAllocator::E_ORDERED ordered){ m_ordered = ordered; }//スタック順更新
public:
	//メソッド
	//メモリ確保
	void* alloc(const std::size_t size, const std::size_t align = DEFAULT_ALIGN) override
...
public:
	...
	//コンストラクタ
	CSmartStackAllocAdp(CSmartStackAllocator& stack, CSmartStackAllocator::E_ORDERED ordered = CSmartStackAllocator::DEFAULT) :
		m_allocator(stack),//スマートスタックアロケータ
		m_ordered(ordered)//スタック順
	{}
	//デストラクタ
...
private:
	//フィールド
	CSmartStackAllocator& m_allocator;//スマートスタックアロケータ
	CSmartStackAllocator::E_ORDERED m_ordered;//スタック順
};
...
public:
	...
	CTempPolyAllocator() = delete;
	//コンストラクタ
	CTempPolyAllocator(CSmartStackAllocator& allocator, const CSmartStackAllocator::E_ORDERED ordered = CSmartStackAllocator::DEFAULT) :
