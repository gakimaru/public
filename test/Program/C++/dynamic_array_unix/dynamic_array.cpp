...
namespace dynamic_array
{
	...
	private:
		//※範囲チェックなし
		inline const value_type* _ref_element(const int index) const { return &m_array[index]; }//要素参照
		...
		inline value_type* _ref_element(const int index){ return &m_array[index]; }//要素参照
	...
	public:
		//※範囲チェックあり
		inline const value_type* ref_element(const int index) const { return index >= 0 && index < static_cast<int>(m_size) ? _ref_element(index) : nullptr; }//要素参照
		...
		inline value_type* ref_element(const int index){ return  const_cast<value_type*>(const_cast<const container*>(this)->ref_element()); }//要素参照
		inline value_type* ref_front(){ return const_cast<value_type*>(const_cast<const container*>(this)->ref_front()); }//先頭要素参照
		inline value_type* ref_bottom(){ return const_cast<value_type*>(const_cast<const container*>(this)->ref_bottom()); }//末尾要素参照
		inline value_type* ref_new(){ return const_cast<value_type*>(const_cast<const container*>(this)->ref_new()); }//新規要素参照
	private:
		inline int _adj_index(const int index) const { return index >= 0 && index < m_maxSize ? index : -1; }//インデックスを範囲内に補正
		inline int _ref_index(const value_type* node) const{ return node - _ref_front(); }//要素をインデックスに変換 ※範囲チェックなし
	public:
		inline int ref_index(const value_type* node) const{ return _adj_index(_ref_index(node)); }//要素をインデックスに変換
	public:
	...
	public:
		...
		{
			if (m_array && m_autoClearAttr == auto_clear_attr_t::AUTO_CLEAR)
				clear();//クリア
			m_array = array;
...
int main(const int argc, const char* argv[])
{
	...
	//テスト①：プリミティブ型の配列を扱う場合
	{
		...
		auto find = [&con](const int val)
		{
			...
			auto ite = con.find_value(val);//探索
			if (ite.isExist())
			{
				printf("%d", *ite);
				++ite;
	...
	//テスト②：ユーザー定義型を扱う場合
	{
...
#if 0
		{
			...
			if (ite.isExist()) printf("ite:[%d] seqNo=%d, value=%d\n", ite.getIndex(), ite->m_key, ite->m_val);
			if (rite.isExist()) printf("rite:[%d] seqNo=%d, value=%d\n", rite.getIndex(), rite->m_key, rite->m_val);
			if (ite_end.isExist()) printf("ite_end:[%d] seqNo=%d, value=%d\n", ite_end.getIndex(), ite_end->m_key, ite_end->m_val);
			if (rite_end.isExist()) printf("rite_end:[%d] seqNo=%d, value=%d\n", rite_end.getIndex(), rite_end->m_key, rite_end->m_val);
		...
		auto find = [&con](const int key)
		{
			...
			auto ite = con.find_value(key);//探索
			if (ite.isExist())
			{
	...
	//テスト⑤：ロック制御を行う
	{
		...
		testThread<dynamic_array::container<ope_t> >("normal container");
		testThread<dynamic_array::container<mt_ope_t> >("multi-thread container");
	}
...
{
	typedef C container_t;//テンプレートパラメータで、ロック制御ありコンテナか、なしコンテナの型を受け取る
