...
const char* TITLE = "OpenGL GLSL4.0 Test 01";

#define SHADER_SOURCE_BASE_PATH "E:\\Work\\GitHub\\public\\test\\Program\\C++\\OpenGL_Test01\\shader\\"
...
const GLint ATTRLOC_VERTEX_COLOR = 1;
...
void drawGLUT()
{
	...
	const GLuint color_buffer_handle = vao_handles[ATTRLOC_VERTEX_COLOR];
	...
	glBindBuffer(GL_ARRAY_BUFFER, color_buffer_handle);
	glVertexAttribPointer(ATTRLOC_VERTEX_COLOR, 4, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLubyte*>(NULL));
...
//	g_UNIFORMLOC_ROTATION_MAT = glGetUniformLocation(g_program_handle, "RotationMatrix");
	if (g_UNIFORMLOC_ROTATION_MAT != INVALID_LOC)
	{
		...
		const glm::mat4 scale_matrix = glm::scale(scale_xy, scale_xy, 1.f);
		const glm::mat4 rotation_matrix = glm::rotate(glm::mat4(1.f), g_rot_angle, glm::vec3(0.f, 0.f, 1.f));
...
GLuint createAndLoadFragmentShaderAll()
{
	...
	{
		const char* shader_source_file_path_list[] = {
			SHADER_SOURCE_BASE_PATH "basic.frag"
		};
		const int shader_source_file_num = countof(shader_source_file_path_list);
		const GLchar** shader_source_file_list = loadShaderSource(shader_source_file_num, shader_source_file_path_list);
		if (shader_source_file_list)
