...
//・プリント処理のスレッド化対応
//　　⇒処理効率の最適化：スレッドのサスペンド（ポーズ）とレジューム（シグナル／イベント）の仕組みを利用し、メッセージのフラッシュ要求時やオブジェクトの削除要求時にのみプリントスレッドが動作するようにする。
//　　⇒スレッドセーフ機構の最適化：ロックを減らし、役割を終えたオブジェクトに対して隠すレッドから is_deleted = true に更新し、プリント処理スレッドが実際の削除を行う。ロックが必要なのは、スレッド情報オブジェクトの追加／削除時のみとする。is_deletedフラグはロックせずに更新する（フラグ更新後に、オブジェクトに一切触れなければ問題がない）。
//・ノーティファイ処理（画面表示処理）対応
//・アサーション対応
